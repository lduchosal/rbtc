...
";
    ...
    assert_eq!(b.version, 0x00000001, "version");
    assert_eq!(b.previous, [0; 32]);
    ...
    assert_eq!(b.time, 0x29AB5F49);
    assert_eq!(b.bits, 0xFFFF001D);
    assert_eq!(b.nonce, 0x1DAC2B7C);
    ...
    assert_eq!(i.sequence, 0xFFFFFFFF);
    assert_eq!(i.signature.content.len(), 0x4D);
    ...
    assert_eq!(i.witness.len(), 0);

    let p : &OutPoint = &i.previous;

    assert_eq!(p.transaction_hash, [0; 32]);
    assert_eq!(p.index, 0);

    let o : &TxOut = t.outputs.get(0).unwrap();

    assert_eq!(o.script_pubkey.content.len(), 0x43);
    ...
    assert_eq!(o.value, 0x00F2052A01000000); // 50BTC
